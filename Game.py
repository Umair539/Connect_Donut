class Game:
    def __init__(self):
        self.rows = 6
        self.cols = 7
        self.amount = 2 #how many in a row
    ...
    def check(self):
        ...
        def diagonal(cols,rows,l,am,x,y):
            ...
            for i in range( len(dcol) +1 -am ):
                ...
                if len(set(r)) == 1 and list(set(r))[0] != 0: #check if list of 4 is the same
                    return True
            ...
            for i in range( len(ucol) +1 -am ):#possible combinations for 'am' in a row in diagonal
                r = []
                for j in range(am):
        ...
        def row(l,cols,lst,am):
            ...
            for i in range(1,cols +2 - am): #for n col, only 'am' placements. indexing backwards so 1:'am'+1
                r = []
                for j in range(am):
        ...
        if row(self.l,self.cols,lst,self.amount) or column(lst,self.amount) or diagonal(self.cols,self.rows,self.l,self.amount,x,y):
            self.w = 2
