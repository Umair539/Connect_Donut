...
def redraw(game,col,Player):#w, 0>carry on,1>draw,2>win
    ...
    if game.p == Player: #highlight
        pygame.draw.rect(win,(80, 80, 80), pygame.Rect(col*100,T,100,game.rows*100))
    
    for i in range(game.cols-1):
        pygame.draw.rect(win, (255,0,255), pygame.Rect(99+100*i,T,2,game.rows*100))
    
    for i in range(game.rows):
        pygame.draw.rect(win, (255,0,255), pygame.Rect(0,99+100*i,game.cols*100,2))
...
def main(n,game,Player):
    ...
    while run:
        ...
        a,b = pygame.mouse.get_pos()
        col = a//100
        redraw(game,col,Player)
            
        if game.w != 0:
            game = n.send(-3)
        
        if game.p != Player: #waiting for other play to make move and return updated game state
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and game.l[col][-1] == 0 and game.p == Player and b>100: #2nd is if there is space in that column 
